...
import { updateFlow, getFlow, run } from "@/api/pipeline.js";
import { getRunningFlowInfo } from "@/api/task.js";
...
onConnect((params) => {
  ...
  params["type"] = "default";
  params["id"] = uuidv4();
  params["data"] = { readOnly: props.readOnly };
  addEdges([params]);
});

watch(runningInfo, (infos) => {
  for (const info of infos.nodeResultList) {
    const { id } = info;
...
const initRunningInfo = () => {
  if (taskId.value !== undefined && taskId.value !== "") {
    getRunningFlowInfo({ id: taskId.value }).then((res) => {
